...
| checkpoint | `~/agg/checkpoint_ensemble_01/blackboard_checkpoint_ensemble_01` | `…/analysis-runs/parallel-tracked-20260918/output/tables` (job 46, provisional) | 16 CPU / 44 G (reference used 32 / 110 G on big-0; measured peak RSS 6.8 GiB, so a standard node suffices), `big-0` | `--allow-incomplete`; `study_lineage.json` is set aside in that root |
