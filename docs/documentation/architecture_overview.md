...
| Metrics | Scientific quantities at four scopes: `StreamingMetric` (one round), `FinalMetric` (one episode), `AggregateMetric` (one grid cell, many episodes), `SweepMetric` (the grid, many cells). The two cross-episode tiers are post-hoc — they read finished episodes off disk, so aggregates are recomputable with a different band, window or fill rule without re-running anything. The aggregation rules (forward-fill absorbed episodes, relabel by winner, percentile bands, roll within then aggregate) are correctness requirements, not preferences. | `metrics/` (`base.py`, `generic.py`, `rolling.py`, `aggregate.py`, `cell.py`, `sweep.py`, `plotting.py`), `games/naming_convention/metrics.py` |
...
| Experiments | Phase 9: N episodes of one resolved config, bounded concurrency, resumable by episode; grid sweeps (`grid:` config section, cartesian product of cells, one shared concurrency/budget pool). **Master-only logging**: workers write episode files and nothing else, the master writes all Comet — one sweep experiment stepped by `episodes_done` (timer heartbeat, progress, ETA, grid-progress image) plus one experiment per cell stepped by `round` (the aggregate curves), created at that cell's completion. Comet is a view, not the store: a run killed at 80% still has correct aggregates for every finished cell, and `mas-cc experiment aggregate` recomputes them from disk. | `experiments/orchestrator.py`, `experiments/console.py`, `experiments/comet_monitor.py`, `experiments/aggregation.py`, `config/grid.py`, `planning/grid_preflight.py` |
...
| CLI | `mas-cc game run`, `mas-cc experiment run|preflight|aggregate`, `mas-cc prompt ...`, `mas-cc analysis empowerment`, etc. | `cli/` |
