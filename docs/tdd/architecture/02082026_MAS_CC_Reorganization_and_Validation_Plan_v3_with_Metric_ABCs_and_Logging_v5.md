...
# Phase 8.0 — Generic metrics framework, metric scopes, and logging policy

## Objective

Introduce a game-independent, extensible metrics system so that each game can
define its own scientific metrics through abstract metric contracts. Metrics are
selected, parameterized, scheduled, stored, and optionally exported to Comet from
the resolved run or experiment configuration. The Naming Convention metrics are
the first concrete fixture, not the definition of the framework.

## Architectural principle

Raw game records are the source of truth. Metrics are versioned derived objects
that consume explicit immutable record views and produce typed outputs. A new
metric must be addable by implementing one metric subclass, registering it, and
selecting it in configuration. It must not require changes to the generic game
loop, provider layer, storage engine, Comet adapter, or experiment orchestrator.

Metric computation and metric emission are separate concerns:

- the **metric implementation** defines what is computed and from which records;
- the **metric instance configuration** defines parameters and execution cadence;
- the **logging/export configuration** defines where outputs are written;
- the **metric access policy** defines the maximum allowed visibility;
- configuration may reduce visibility, but it may never override a metric's
  privacy restriction to export protected values.

The framework must distinguish:

- **agent metrics**: one value per selected agent;
- **population/system metrics**: values computed directly from the joint game state;
- **interaction metrics**: values computed from one transition or interaction;
- **episode metrics**: summaries over one trajectory;
- **experiment metrics**: aggregations across episodes or conditions;
- **aggregate metrics**: explicit reductions of lower-level metric outputs;
- **online metrics**: incrementally updated during execution;
- **offline metrics**: recomputed from persisted records;
- **visualizations**: optional renderers over stored metric outputs, never part of
  the game transition.

## Abstract metric contracts

The exact Python spelling may change, but production code must use abstract base
classes or an equivalently strict typed interface. The intended hierarchy is:
...
class Metric(ABC, Generic[InputT, OutputT]):
    ...
    @abstractmethod
    def compute(self, value: InputT) -> OutputT: ...

class OnlineMetric(Metric[InputT, OutputT], Generic[InputT, OutputT, StateT]):
    @abstractmethod
    def initialize(self, context: "MetricContext") -> StateT: ...
    ...
    def update(
        ...
        state: StateT,
        record: InputT,
    ) -> tuple[StateT, tuple[OutputT, ...]]: ...

    @abstractmethod
    def finalize(self, state: StateT) -> tuple[OutputT, ...]: ...
...
class EpisodeMetric(Metric["EpisodeMetricView", OutputT], ABC):
    pass

class ExperimentMetric(Metric["ExperimentMetricView", OutputT], ABC):
    pass
...
Concrete metrics are subclasses; parameter choices create immutable instances:

```python
RollingActionShare(action="Q", window_interactions=30)
RollingActionShare(action="M", window_interactions=30)
AgentStateIndicator(target_state="two_items")
MeanAcrossAgents(source_metric="agent_state_indicator")
```

An aggregate over agent metrics is therefore explicit and compositional. The
framework must also allow direct system metrics for quantities that cannot be
represented correctly as independent per-agent reductions, such as consensus,
network connectivity, polarization, or global coordination.

All outputs include metric name, version, instance ID, scope, entity/episode ID,
logical time, parameter hash, value schema, and provenance hash.

## Metric ownership and registration

1. The generic package owns the abstract classes, `MetricSpec`, typed input views,
   registry, execution, persistence, validation, sink adapters, and manifests.
2. Each game owns concrete scientific metrics under
   `games/<game>/metrics.py` or a game-local metrics package.
3. Cross-game operational metrics such as latency, token use, retries, cost, and
   throughput live in a generic operational metrics package.
4. Cross-game scientific estimators may live in `analysis/metrics/`, but extraction
   of their variables remains explicit and configured.
5. `MetricRegistry` resolves `name@version` to a metric class and constructs an
   immutable parameterized metric instance.
6. Games expose available and recommended metric sets. Experiments explicitly
   select, remove, or parameterize them.
7. Metric implementations never import Comet or write files directly. They return
   typed values to generic sinks.

## Run and episode configuration

The selected metric set is part of the resolved run or experiment configuration.
Episodes inherit that metric configuration so all episodes in one experimental
condition are comparable. Episode-level overrides are allowed only when explicitly
specified as part of the experiment design and must appear in the resolved metric
manifest and episode fingerprint.
...
metrics:
  ...
  instances:
    - id: action_share_q
      metric: naming_convention.action_share@1
      mode: both
      scope: population
      parameters:
        action: Q
    ...
    - id: fraction_agents_in_two_item_state
      metric: generic.mean_across_agents@1
      mode: offline
      scope: population
      parameters:
        source_metric:
          metric: toy.agent_state_indicator@1
          parameters:
            target_state: two_items
      sinks:
        local:
          enabled: true
        comet:
          enabled: true
          key: population/fraction_two_item_state
          summary_only: true
```

The configuration resolver must fail before execution when:

- a metric name/version is unknown;
- parameters are invalid;
- the game does not expose required records;
- an entity selector is invalid;
- an online cadence is impossible for the selected metric;
- Comet is requested for a non-scalar or restricted metric;
- a requested sink exceeds the metric's access/export policy.

## Computation, logging, and Comet policy

Metric computation is independent from logging. A metric may be:

- computed and stored locally but not logged remotely;
- computed online only for a stopping condition;
- emitted to console for debugging;
- sent to Comet as a scalar time series;
- sent to Comet only as an episode or experiment summary;
- computed offline after the run without any live logging.

The generic metric runner sends values to configured sinks. Comet export must be
opt-in per metric instance unless a run-level default explicitly enables an
approved metric set. Only scalar or explicitly supported aggregate values may be
sent to Comet. Per-agent identifiers, private memories, evaluator-only records,
large vectors, tables, and sensitive values remain local by default.

The metric class declares an upper-bound export capability, for example:
...
The run configuration may choose a stricter sink policy but cannot choose a more
permissive one. The resolved configuration and metric manifest record the final
effective policy.

## Data access and scientific safety

Each metric declares the narrowest immutable input view it requires:
`AgentMetricView`, `SystemMetricView`, `InteractionMetricView`,
`EpisodeMetricView`, or `ExperimentMetricView`. Metrics may not reach into mutable
live game objects. If a required observable is absent, the game record schema must
be versioned deliberately.

Evaluator-only or private state may be used for permitted scientific analysis,
but metric values never leak into ordinary agent prompts, transitions, or control
policies unless the experiment explicitly defines that metric as an observable
control input. Such use must be separately configured and audited.

## Storage and provenance

Every run saves a metric manifest containing:

- selected metric classes, names, versions, and instance IDs;
- scopes, entity selectors, parameters, and parameter hashes;
- required input fields and input schema versions;
- online/offline execution mode and cadence;
- output schemas and artifact locations;
- requested and effective sink policies;
- Comet keys and summary/time-series policy when enabled;
- access level and export capability;
- successful, skipped, failed, and non-computable status with reasons.

Metric outputs are stored by stable identity:

```text
metrics/
├── manifest.json
├── values/
│   ├── action_share_q.parquet
│   ├── agent_7_q_count.parquet
│   └── fraction_agents_in_two_item_state.parquet
├── episode_summaries.parquet
├── experiment_summaries.parquet
├── comet_export_manifest.json
└── plots/
```

A convenience wide table may additionally be produced, but it is not canonical
because games and metric versions can have different output schemas.

## Baseline Naming Convention metric fixture

The first concrete game metric set must implement:

- rolling action share for every legal action;
- rolling coordination rate;
- dominant action and dominant-action share;
- resolved convention and first-resolution time;
- per-agent action counts and participation counts;
- payoff summaries;
- population aggregates over agent-level metrics;
- episode-level and across-episode aggregation.

These remain game-owned subclasses of the generic contracts. They must not be
hard-coded into the runtime, storage, reporting, or Comet integration.

## Tasks

1. Implement `MetricSpec`, `MetricValue`, `MetricState`, `MetricContext`, typed
   metric input views, and versioned output-schema records.
2. Implement the abstract `Metric`, `OnlineMetric`, `AgentMetric`, `SystemMetric`,
   `InteractionMetric`, `EpisodeMetric`, `ExperimentMetric`, and
   `AggregateMetric` classes.
3. Implement `MetricRegistry` and immutable construction of parameterized metric
   instances from `name@version`.
4. Implement configuration schemas for metric selection, entity selectors,
   cadence, sinks, Comet keys, and local persistence.
5. Let every game expose available and recommended metrics without silently
   enabling them in scientific runs.
6. Validate metric requirements against game and storage schemas during preflight.
7. Execute online metrics from immutable post-transition records.
8. Execute offline metrics only from persisted versioned artifacts.
9. Implement aggregation over agent metrics and direct system metrics as separate
   supported paths.
10. Persist manifests, parameter hashes, values, statuses, failures, and effective
    export policies.
11. Add generic console, local-file, in-memory, and Comet sink adapters. Metric
    subclasses must contain no sink-specific code.
12. Support metric-specific optional plot/report renderers through a separate
    registry.
13. Add the Naming Convention baseline metrics as the first contract fixture.
14. Add a toy agent metric plus a population aggregate over that metric.
15. Test that registering a new metric requires no change to runtime, provider,
    storage, experiment orchestration, or Comet code.
16. Test metric versioning, invalid entity selectors, missing inputs, deterministic
    replay, online/offline parity, sink failures, cadence, privacy, and export
    restrictions.
17. Test that Comet can be enabled for one metric and disabled for another in the
    same run, with local persistence continuing for both.
18. Test that episode inheritance and explicit episode overrides are recorded in
...
Checkpoint shards are recoverable intermediate state. Raw compacted records under
`data/` remain the scientific source of truth. Metrics must be reproducible from
those records whenever their declared inputs are available.
...
├── resolved_metric_config.yaml
├── input_schema_validation.json
├── metric_values/
├── online_offline_parity.csv
...
- Instantiate the same metric class with two parameter sets and confirm distinct
  instance IDs and provenance hashes.
- Compute one agent metric for a selected agent and aggregate the same metric over
  all agents.
...
- Confirm computation cadence and logging cadence are independent.
- Confirm episode metric configuration is inherited and any override changes the
  episode fingerprint.
- Confirm online and offline recomputation agree for deterministic metrics.
- Confirm metric values never enter prompts or transitions without an explicit,
  audited control-observation configuration.

## Acceptance criterion

A researcher can introduce a new game-specific or cross-game metric by implementing
one versioned abstract-metric subclass, registering it, and selecting a parameterized
instance in configuration. The framework supports per-agent, direct system,
interaction, episode, experiment, and aggregate metrics. Each run explicitly
configures which metrics are computed, their cadence, where they are stored, and
whether approved scalar outputs are sent to Comet. Metric computation remains
independent of logging, and privacy/export constraints cannot be weakened by run
configuration. No new metric requires changes to the game loop, provider,
storage, experiment orchestrator, or Comet adapter.
