...
17. As the final Phase 7 end-to-end inspection, run the Naming Convention Game
   ...
   least three rounds with exactly one prompt/provider attempt per round, and
   save the local inspection artifacts plus an inspectable Comet run reference
   or summary. This is an integration demonstration, not a substitute for the
   mock-based unit and resume checks.
...
- Inspect the final Naming Convention Game Comet run: it must contain at least
  three rounds and one prompt/provider attempt in each round, and its aggregate
  metrics must agree with the corresponding local logs and `comet_summary.json`.
...
three-or-more-round Naming Convention Game run using both Comet and normal local
logging, with exactly one prompt/provider attempt per round.
