(() => {
  ...
  function sparkline(points, key = 'truth_share', width = 150, height = 42) {
    ...
    const interventions = points.map((point, index) => point.controller_post ? `<line class="intervention-marker" x1="${x(index)}" y1="${top}" x2="${x(index)}" y2="${top+plotHeight}"><title>Controller post at update ${index+1}</title></line>` : '').join('');
    ...
    return `<svg class="sparkline" viewBox="0 0 ${width} ${height}" role="img" aria-label="${esc(key)} trajectory">${grid}${interventions}<polyline points="${coordinates}"></polyline>${dots}<text class="axis-label" x="${left}" y="${height-3}">1</text><text class="axis-label" x="${width-right}" y="${height-3}" text-anchor="end">${points.length}</text></svg>`;
  }
  ...
  function renderStudy() {
    ...
    filterOptions('filter-target', study.cells.map(controllerTargetLabel));
  ...
  function renderCellTable() {
    let cells = state.study.cells.filter(cell =>
      ...
      (!$('filter-target').value || controllerTargetLabel(cell) === $('filter-target').value) &&
    ...
    $('cell-table').innerHTML = `<thead><tr><th>Config / cell</th><th>Condition</th><th>Target</th><th>ρ</th><th>b</th><th>Scientific outcomes</th><th>Live activity</th><th>SLURM task</th><th></th></tr></thead><tbody>${cells.map(cell => `<tr><td><b>${esc(cell.config_name)}</b><br><span class="meta">${esc(cell.cell_id)}</span></td><td>${esc(unavailable(cell.parameters.controller_condition))}</td><td>${esc(controllerTargetLabel(cell))}</td><td>${esc(unavailable(cell.parameters.rho))}</td><td>${esc(unavailable(cell.parameters.b))}</td><td>${cell.outcome_counts.completed}/${cell.expected_episodes} durable complete<br><span class="meta">${cell.outcome_counts.failed} failed · ${cell.outcome_counts.aborted} aborted · ${cell.outcome_counts.unknown} unknown</span></td><td>${cell.activity_counts.running + cell.activity_counts.advancing} running<br><span class="meta">${cell.activity_counts.started_unchanged} inactive stream</span></td><td>${cell.scheduler ? `${statusBadge(cell.scheduler.state)}<br><span class="meta">array ${cell.scheduler.array_index} · ${esc(unavailable(cell.scheduler.node))} · ${esc(unavailable(cell.scheduler.elapsed))}</span>` : '<span class="unavailable">Unavailable</span>'}</td><td><button class="open-cell" data-cell="${esc(cell.qualified_id)}">Open</button></td></tr>`).join('')}</tbody>`;
    document.querySelectorAll('.open-cell').forEach(button => button.addEventListener('click', () => openCell(button.dataset.cell)));
  }

  async function setCellTab(id) {
    state.cellTab = id;
  ...
  function renderCell(cell) {
    ...
    const primary = [['Controller condition', cell.parameters.controller_condition], ['Controller target', controllerTargetLabel(cell)], ['ρ', cell.parameters.rho], ['b', cell.parameters.b], ['Task', cell.parameters.task_id], ['Population', cell.parameters.population_size], ['Rounds', cell.parameters['game.options.rounds']], ['Truth', cell.parameters.ground_truth]];
  ...
  function populateTimeline(timeline) {
    ...
    const indices = timeline.rounds.map(item => Number(item.round_index));
    round.min = indices.length ? Math.min(...indices) : 0;
  ...
  function updateStepRange() {
    ...
    if (!selected) return;
    $('round-value').value = Number(selected.round_index) + 1;
