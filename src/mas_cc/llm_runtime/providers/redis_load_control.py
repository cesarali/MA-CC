...
# Shared prelude for every script. ARGV[1] is always ``now`` and ARGV[2..6]
...
#   ARGV[5] token  ARGV[6] node  ARGV[7] worker
#   ARGV[8] lease_seconds  ARGV[9] polling_seconds  ARGV[10] target_rpm
_ACQUIRE = _PRELUDE + r"""
local token = ARGV[5]
local node = ARGV[6]
local worker = ARGV[7]
local lease_seconds = tonumber(ARGV[8])
local polling = tonumber(ARGV[9])
local target_rpm = tonumber(ARGV[10])
...
#   ARGV[5] token  ARGV[6] lease_seconds
_RENEW = _PRELUDE + r"""
local token = ARGV[5]
...
#   ARGV[5] token  ARGV[6] event_id  ARGV[7] node  ARGV[8] worker
#   ARGV[9] success (0/1)  ARGV[10] retryable (0/1)  ARGV[11] status_code ('' = none)
#   ARGV[12] latency_seconds
#   ARGV[13] local_failure_threshold  ARGV[14] local_cooldown_seconds
#   ARGV[15] global_min_samples  ARGV[16] global_failure_ratio
#   ARGV[17] global_cooldown_seconds  ARGV[18] decrease_factor
#   ARGV[19] minimum_concurrency  ARGV[20] maximum_concurrency
#   ARGV[21] increase_step  ARGV[22] increase_interval_seconds
_RELEASE = _PRELUDE + r"""
local token = ARGV[5]
local event_id = ARGV[6]
local node = ARGV[7]
local success = ARGV[9] == '1'
local retryable = ARGV[10] == '1'
...
if ARGV[11] ~= '' then status = tonumber(ARGV[11]) end
redis.call('ZADD', KEYS[5], now, event_id)
redis.call('HSET', KEYS[6], event_id, cjson.encode({
  event_id = event_id, at = now, worker = ARGV[8], node = node,
  success = success, retryable = retryable, status_code = status,
  latency_seconds = math.max(0, tonumber(ARGV[12])),
}))
if retryable then
...
if retryable and local_fails >= tonumber(ARGV[13]) then
  redis.call('HSET', KEYS[9], node, now + tonumber(ARGV[14]))
end

local limit = tonumber(redis.call('GET', KEYS[1]))
if retryable and total >= tonumber(ARGV[15]) and fails / total >= tonumber(ARGV[16]) then
  local cooldown = tonumber(ARGV[17])
  ...
  if now - meta_number('last_decrease_at') >= cooldown then
    local reduced = math.max(tonumber(ARGV[19]), math.floor(limit * tonumber(ARGV[18])))
    redis.call('SET', KEYS[1], reduced)
    redis.call('HSET', KEYS[10], 'last_decrease_at', now)
  end
elseif success and fails == 0 and now - meta_number('last_increase_at') >= tonumber(ARGV[22]) then
  local grown = math.min(tonumber(ARGV[20]), limit + tonumber(ARGV[21]))
        ...
        async with self._acquire_gate:
            ...
            while deadline is None or time.monotonic() < deadline:
                ...
                if delay <= self.config.polling_seconds:
                    ...
                    delay = self._jitter.uniform(0.75 * contention_delay, 1.25 * contention_delay)
                await asyncio.sleep(
                    min(
                        max(0.01, delay),
                        max(0, deadline - time.monotonic()) if deadline else max(0.01, delay),
                    )
                )
        raise ProviderCoordinationUnavailable(
