...
class SolvabilityConfig:
    ...
    def nominal_calls(self) -> int:
        ...
        phase_c = len(self.heldout_tasks) * (
            2 * self.heldout_repetitions
            + self.population_size * self.heldout_repetitions
        )
...
def load_config(path: str | Path) -> SolvabilityConfig:
    ...
    if (provider.type, provider.model) != ("university", "gwdg/openai-gpt-oss-120b"):
        raise SolvabilityConfigError(
            "calibration requires university/gwdg/openai-gpt-oss-120b"
        )
    ...
    development = tuple(
        str(item) for item in tasks.get("development", ("task_001", "task_002"))
    )
    ...
    if len(development) < 2 or not heldout or set(development) & set(heldout):
        raise SolvabilityConfigError(
            "use at least two development tasks and disjoint held-out tasks"
        )
    ...
    if config.workers > provider.request_concurrency:
        raise SolvabilityConfigError(
            "workers cannot exceed provider request_concurrency"
        )
    return config
