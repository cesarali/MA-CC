"""Exact production game-prompt adapter for isolated evidence conditions."""
...
def render(task: RelationalTask, spec: CallSpec) -> RenderedCall:
    identity = (
        ...
        else f"Agent {int(spec.agent_id.rsplit('_', 1)[1])}"
    )
    prompt = build_relational_ballot_prompt(
        ...
        known_facts=tuple(
            ...
            for fact_id in spec.evidence_ids
        ),
...
def parse(task: RelationalTask, spec: CallSpec, response: str) -> dict[str, Any]:
    try:
        ...
        answer = spec.option_mapping.get(ballot.vote, ballot.vote)
        ...
        if (
            ...
            raise ValueError("shared_fact_id is unavailable in this condition")
        return {
            "parse_success": True,
            "selected_letter": ballot.vote,
            "parsed_semantic_answer": answer,
            "correct": answer == task.correct_relation,
