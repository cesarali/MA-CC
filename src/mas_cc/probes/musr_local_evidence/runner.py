...
from .analysis import comparison_family, render_plots, summarize_by_latent_coverage, summarize_doses, summarize_doses_by_agent, summarize_prompt_equivalence, terminal_rows, write_csv
...
def prepare(config: LocalEvidenceProbeConfig, output_dir: Path | None = None) -> tuple[Path,ProbePlan,dict[str,Any]]:
    root=Path(output_dir or config.output_dir); root.mkdir(parents=True,exist_ok=True)
    plan=build_plan(config); payload=preflight_payload(config,plan)
    ...
    task_dir=root/"task"; task_dir.mkdir(parents=True,exist_ok=True)
    base=Path(plan.task.source_path.split("|",1)[0]); distribution=Path(plan.task.source_path.split("|",1)[1])
    shutil.copy2(base,task_dir/"task_001.json"); shutil.copy2(distribution,task_dir/"distribution_N12.json")
    evidence_rows=[{"evidence_id":card,"latent_fact_id":latent,"text":plan.task.fact_text(card)} for latent,cards in (plan.task.supporting_fact_groups or {}).items() for card in cards]
    write_csv(task_dir/"evidence_catalog.csv",evidence_rows)
    views=[{"agent_id":agent,"evidence_ids":"|".join(plan.task.known_facts(f"agent_{agent:03d}")),"card_count":len(plan.task.known_facts(f"agent_{agent:03d}"))} for agent in config.agents]
    write_csv(task_dir/"agent_initial_views.csv",views)
    (root/"config.yaml").write_text(yaml.safe_dump(config.to_dict(),sort_keys=False),encoding="utf-8")
    pre=root/"preflight"; pre.mkdir(parents=True,exist_ok=True)
    write_json_atomic(pre/"preflight.json",payload); write_json_atomic(pre/"pricing_snapshot.json",quote.to_dict()); write_json_atomic(pre/"call_plan.json",[spec.to_dict() for spec in plan.calls]); write_json_atomic(root/"evidence_dose/dose_definitions.json",list(plan.dose_definitions))
    approval=sha256_object({"config":config.to_dict(),"preflight":payload})
    (pre/"preflight_id.txt").write_text(approval+"\n",encoding="utf-8")
    report=["# MuSR local evidence probe preflight","",f"- Passed: **{payload['passed']}**",f"- Logical calls: {payload['calls']['total']} (60 paired-prompt + 63 dose)",f"- Estimated input tokens: {payload['tokens']['estimated_input_total']:,}",f"- Maximum output-token ceiling: {payload['tokens']['maximum_output_total']:,}",f"- Conservative cost: {estimated_cost.amount:.6f} {estimated_cost.unit}",f"- Pricing source: {quote.source} ({quote.status})",f"- Workers: {payload['concurrency']}",f"- Approximate wall time: {payload['estimated_wall_seconds']/60:.1f} minutes","","## Checks"]
    report.extend(f"- [{'PASS' if row['passed'] else 'FAIL'}] {row['check']}: {row['detail']}" for row in payload['checks'])
    (pre/"report.md").write_text("\n".join(report)+"\n",encoding="utf-8")
    manifest={"schema_version":1,"probe":"musr_local_evidence","status":"planned","config_sha256":sha256_object(config.to_dict()),"call_plan_sha256":payload["call_plan_sha256"],"dose_definitions_sha256":payload["dose_definitions_sha256"],"prompt_hashes_sha256":payload["prompt_hashes_sha256"],"task_hashes":payload["task_hashes"],"pricing_snapshot_sha256":sha256_object(quote.to_dict()),"provider":config.provider.type,"model":config.provider.model,"requested_decoding":{"temperature":config.provider.temperature,"max_output_tokens":config.provider.max_output_tokens,"transport_retries":config.provider.max_retries},"mas_cc_git":_git(Path.cwd())}
    write_json_atomic(root/"manifest.json",manifest)
    (root/"README.md").write_text("# MuSR local evidence probe 01\n\nSee `analysis/local_evidence_probe_report.md` after execution.\n",encoding="utf-8")
    return root,plan,payload


def _report(plan: ProbePlan, pair_summary: list[dict[str,Any]], dose_summary: list[dict[str,Any]], pairs: list[dict[str,Any]], root: Path) -> str:
    pooled=next(row for row in pair_summary if row["agent_id"]=="pooled")
    zero=next(row for row in dose_summary if row["cards"]==0); full=next(row for row in dose_summary if row["cards"]==27)
    example_validation=next(row for row in terminal_rows(read_journal(root/"prompt_equivalence/raw_calls.jsonl")) if row.get("agent_id")==1 and comparison_family(row)=="validation")
    example_game=next(row for row in terminal_rows(read_journal(root/"prompt_equivalence/raw_calls.jsonl")) if row.get("pair_id")==example_validation.get("pair_id") and comparison_family(row)=="game_init")
    def messages(row: Mapping[str,Any]) -> str:
        return "\n\n".join(f"[{m['role']}]\n{m['content']}" for m in row["messages"])
    pair_lines=["| Agent | Validation truth | Game-init truth | Paired disagreement |","|---:|---:|---:|---:|"]+[f"| {r['agent_id']} | {r['validation_truth_rate']:.1%} | {r['game_init_truth_rate']:.1%} | {r['paired_disagreement_rate']:.1%} |" for r in pair_summary]
    dose_lines=["| Cards | Mean latent facts | n | Truth | Truth rate | 95% CI |","|---:|---:|---:|---:|---:|---:|"]+[f"| {r['cards']} | {r['mean_latent_facts_covered']:.1f} | {r['n']} | {r['truth_choices']} | {r['truth_rate']:.1%} | [{r['ci95_low']:.1%}, {r['ci95_high']:.1%}] |" for r in dose_summary]
    nested=next(row for row in plan.dose_definitions if row["agent_id"]==1 and row["dose"]==9)
    monotone=all(a["truth_rate"]<=b["truth_rate"] for a,b in zip(dose_summary,dose_summary[1:]))
    by_cards={row["cards"]:row for row in dose_summary}
    first_improvement=next((cards for cards in sorted(by_cards) if by_cards[cards]["truth_rate"]>1/3),None)
...
def analyze(root: Path, plan: ProbePlan) -> dict[str,Any]:
    equivalence=read_journal(root/"prompt_equivalence/raw_calls.jsonl"); dose=read_journal(root/"evidence_dose/raw_calls.jsonl")
    pairs,pair_summary=summarize_prompt_equivalence(equivalence); observations,dose_summary=summarize_doses(dose)
    by_agent=summarize_doses_by_agent(observations); by_coverage=summarize_by_latent_coverage(observations)
    write_csv(root/"prompt_equivalence/paired_results.csv",pairs); write_csv(root/"prompt_equivalence/summary.csv",pair_summary)
    write_csv(root/"evidence_dose/observation_level_results.csv",observations); write_csv(root/"evidence_dose/summary.csv",dose_summary)
    tables=root/"analysis/tables"; write_csv(tables/"prompt_equivalence_table.csv",pair_summary); write_csv(tables/"dose_curve_table.csv",dose_summary); write_csv(tables/"dose_curve_by_agent.csv",by_agent); write_csv(tables/"latent_coverage_curve.csv",by_coverage); write_csv(tables/"evidence_coverage_table.csv",plan.dose_definitions)
    render_plots(pair_summary,dose_summary,observations,root/"analysis/figures")
    report=_report(plan,pair_summary,dose_summary,pairs,root); report_path=root/"analysis/local_evidence_probe_report.md"; report_path.parent.mkdir(parents=True,exist_ok=True); report_path.write_text(report,encoding="utf-8")
    expected={spec.call_id for spec in plan.calls}
    terminal={str(row["call_id"]):row for row in terminal_rows((*equivalence,*dose))}
    execution={
    ...
    successful_rows=[row for row in terminal.values() if row.get("parse_success") is True]
    usage={
        "requests":len(successful_rows),
        "input_tokens":sum(int((row.get("usage") or {}).get("input_tokens") or 0) for row in successful_rows),
        "output_tokens":sum(int((row.get("usage") or {}).get("output_tokens") or 0) for row in successful_rows),
        "transport_retries":sum(int(row.get("transport_retries") or 0) for row in successful_rows),
    }
    artifacts={str(path.relative_to(root)):sha256_file(path) for path in sorted(root.rglob("*")) if path.is_file() and path.name!="manifest.json"}
    manifest=json.loads((root/"manifest.json").read_text())
    manifest.update({
        "status":"complete" if execution["successful"]==len(expected) else "incomplete",
        "execution":execution,
        "artifact_hashes":artifacts,
        "provider":successful_rows[0].get("provider") if successful_rows else manifest.get("provider"),
        "model":successful_rows[0].get("model") if successful_rows else manifest.get("model"),
    ...
    write_json_atomic(root/"manifest.json",manifest)
    return {"pair_summary":pair_summary,"dose_summary":dose_summary,"report":str(report_path),"execution":execution,"usage":usage}
...
async def run(
    ...
    *,
    ...
    if not payload.get("passed"): raise RuntimeError("preflight failed; refusing provider calls")
    approved = (
    ...
    for key in ("call_plan_sha256","dose_definitions_sha256","prompt_hashes_sha256","task_hashes"):
        if current[key] != payload[key]:
            raise RuntimeError(f"current probe design does not match approved preflight: {key}")
    if approved != expected_approval:
        raise RuntimeError(
            "probe run requires approve_preflight matching preflight/preflight_id.txt"
        )
    execution=await execute_plan(config,plan,root); result=analyze(root,plan)
    artifacts={str(path.relative_to(root)):sha256_file(path) for path in sorted(root.rglob("*")) if path.is_file() and path.name!="manifest.json"}
    manifest=json.loads((root/"manifest.json").read_text()); manifest.update({"status":"complete" if execution["successful"]==len(plan.calls) else "incomplete","execution":execution,"provider":config.provider.type,"model":config.provider.model,"artifact_hashes":artifacts}); manifest["manifest_content_sha256"]=sha256_object(manifest); write_json_atomic(root/"manifest.json",manifest)
