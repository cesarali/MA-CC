...
5. Add configurable deterministic audit sampling.
6. Load Comet credentials only from the configured environment and keep Comet optional.
7. Log aggregated metrics remotely; keep complete scientific/private traces local.
8. Integrate the Phase 4 runtime budget guard and persist reservations,
   reconciliations, accumulated cost, accounting unit, and price-snapshot hash.
9. Restore accumulated usage and reservations safely on resume.
10. Persist prompt family/version, definition hash, and instance hash per attempt.
11. Persist block name/version, binding state, rendered content, and token count
   under the configured audit-sampling policy.
12. Mark sensitive/private blocks and prevent them from being sent to Comet by default.
13. Restore no mutable prompt state from checkpoints; reconstruct immutable bound
   prompts from state and resolved configuration.
14. Attribute estimated and actual usage to prompt stage and definition hash.
...
A long run is observable and restartable, and every sampled LLM attempt can be
traced from game observation through bound blocks and compiled messages without
leaking private content to remote monitoring.

---

# Phase 8 — Control policies and information-theoretic analysis
...
conda run -n MA-CC mas-cc control demo \
  ...
  --output-dir inspection/phase_08_v3/control

conda run -n MA-CC mas-cc analyze information \
  --config configs/runs/information_estimator_validation.yaml \
  --output-dir inspection/phase_08_v3/analysis
```

## Inspection artifacts

```text
inspection/phase_08_v3/
├── report.md
├── manifest.json
├── control/
