...
from mas_cc.probes.musr_prompt_solvability.design import packet_definitions,phase_a,phase_b,phase_c
...
def test_design_separates_development_and_heldout_and_counts_calls():
    config=load_config(CONFIG); tasks=load_tasks(config)
    ...
    b=phase_b(tasks,config.development_tasks,"P3",config.packet_repetitions,config.seed)
    c=phase_c(tasks,config.heldout_tasks,"P3","F9",config.heldout_repetitions,12,config.seed)
...
def test_prompt_ablation_is_compositional_and_p0_is_unchanged():
    config=load_config(CONFIG); tasks=load_tasks(config); specs=phase_a(tasks,("task_001",),1,config.seed)
    by_variant={spec.prompt_variant:render(tasks[spec.task_id],spec) for spec in specs}
    text={key:"\n".join(m.content for m in value.messages) for key,value in by_variant.items()}
    ...
    assert all("candidate_scores" not in value and "gold_answer" not in value for value in text.values())
...
def test_full_profile_packets_have_fixed_breadth_and_redundancy():
    config=load_config(CONFIG); tasks=load_tasks(config); definitions=packet_definitions(tasks)
    for task_id,packets in definitions.items():
        ...
        assert all(len({card_group[card] for card in packets[name]})==9 for name in packets)


def test_selection_rules_prefer_simpler_near_ties_and_smallest_passing_packet():
    prompts=[{"prompt_variant":"P0","truth_rate":.85,"parse_rate":1.0},{"prompt_variant":"P1","truth_rate":.89,"parse_rate":1.0},{"prompt_variant":"P2","truth_rate":.88,"parse_rate":1.0},{"prompt_variant":"P3","truth_rate":.90,"parse_rate":1.0}]
    assert select_prompt(prompts)=="P0"
    packets=[{"packet_variant":"F9","truth_rate":.79,"parse_rate":1.0},{"packet_variant":"F18","truth_rate":.85,"parse_rate":1.0},{"packet_variant":"F27","truth_rate":.95,"parse_rate":1.0}]
