...
from mas_cc.probes.musr_private_redistribution.assignment import (
    build_assignment,
    structural_summary,
)
...
def test_natural_audit_matches_known_structure():
    ...
    summaries = [
        structural_summary(build_assignment(task, "NAT", cfg.seed))
...
def test_redistributions_have_six_cards_exact_breadth_and_holder_profiles():
    cfg = load_config(CONFIG)
    tasks = load_tasks(cfg)
    amap = assignments(tasks, cfg.seed)
    expected = {
        "R2": sorted([3] * 6 + [2] * 3),
        "R3": [4] * 9,
        "R4": sorted([6] * 3 + [5] * 6),
    ...
    for task_id, task in tasks.items():
        for regime in ("R2", "R3", "R4"):
            ...
            if regime == "R4":
                assert all(row["num_fully_scoreable_allocations"] == 0 for row in rows)


def test_assignments_are_seeded_and_reproducible():
    cfg = load_config(CONFIG)
    task = load_tasks(cfg)["task_001"]
    first = build_assignment(task, "R3", cfg.seed)
    second = build_assignment(task, "R3", cfg.seed)
    different = build_assignment(task, "R3", cfg.seed + 1)
    assert first == second
...
def test_call_plan_has_492_calls_and_matched_private_mappings():
    ...
    amap = assignments(tasks, cfg.seed)
    specs = call_plan(
        tasks, amap, cfg.private_repetitions, cfg.endpoint_repetitions, cfg.seed
    )
        ...
        for agent in range(1, 13):
            for repetition in range(3):
                group = [
                    ...
                    and s.agent_id == agent
                    and s.repetition == repetition
                ]
                assert len(group) == 4
...
def test_selection_prefers_largest_difficult_regime():
    rows = [
        {"regime": "R2", "truth_rate": 0.2},
        {"regime": "R3", "truth_rate": 0.45},
        {"regime": "R4", "truth_rate": 0.55},
    ]
